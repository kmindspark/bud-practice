...
class PaxosProposer
  ...
  bloom do
    ...
    #If any Acceptors had previously accepted any proposal, then they'll have sent their values to the Proposer, who now must set 
    #the value of its proposal, v, 
    #to the value associated with the highest proposal number reported by the Acceptors, let's call it z. If none of the 
    #Acceptors had accepted a proposal up to this 
    #point, then the Proposer may choose the value it originally wanted to propose, say x[17]. The Proposer sends an 
    #Accept message, (n, v), to a Quorum of Acceptors 
    #with the chosen value for its proposal, v, and the proposal number n (which is the same as the number contained 
    #in the Prepare message previously sent to the Acceptors).
