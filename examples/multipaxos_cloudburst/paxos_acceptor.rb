...
class PaxosAcceptor
  ...
  bloom do
    ...
    all_accept_val <- (accept * max_promise_id * all_accept_val).combos {|a, pid, aav| aav if (pid.key == a.val[2] and aav.slot == pid.key)}
    all_promise_id <- (accept * max_promise_id * all_promise_id).combos {|a, pid, api| api if (pid.key == a.val[2] and api.key == pid.key) }

    #sink2 <= all_accept_val {|a| [a.slot, 1] if ding(a.slot)}
  end
