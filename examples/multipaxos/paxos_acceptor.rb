...
class PaxosAcceptor
  ...
  bloom do
    ...
    #promise <~ (prepare * max_promise_id * max_accept_val).pairs {|p, mpi, mav| [@proposer, [p.val[0], false, false, mpi.val, mav.val, p.val[1]]] if p.val[0] <= mpi.val }
    #promise <~ (prepare * max_promise_id * existing_val).pairs {|p, mpi, mav| [@proposer, [p.val[0], false, false, mpi.val, 0, p.val[1]]] if p.val[0] <= mpi.val}
